...
interaction.plot(x.factor = alldata$time, trace.factor = alldata$Random_ID, response=alldata$Pro_pep)
...
diagnosis.table <- alldata[,c("Random_ID","group")]
...
###counting number of nont1d patients who changed status over draw dates
###by_patient is the function name, and it takes in the id variable and the dataset name as arguments
by_patient<-function(ID,data){
  ...
  temp<-lapply(unique(ID), function(x){
    ...
    #dat.temp<-subset(nont1d,nont1d$Random_ID==146158)
    #this creates a new variable called row num and counts the visit number
    dat.temp$row_num<-rep(1:nrow(dat.temp))
    #this creates the total number of visits for patient x 
    dat.temp$visit_total<-max(dat.temp$row_num,na.rm=T)
    ###new code
    tab<-table(dat.temp$group)
    dimension<-dim(tab)
    dat.temp$change<-0
    dat.temp$change[dimension>1]<-1
    dat.temp
   #  count <- 0
   # for(i in 1:nrow(dat.temp)){
   #  if(dat.temp$group[i]!= dat.temp$group[i-1]) count = count +1
   #  }
   #  dat.temp$count<- count
    dat.temp
  })
...
dat.final<- by_patient (nont1d$Random_ID,nont1d)

check <- c("Random_ID","Draw_Date","group","change")
View(dat.final[check])

##create a table of how many patients changed and didn't
nont1d_changes <- dat.final%>% 
  group_by(Random_ID) %>%
  filter(row_number(Draw_Date)==1)
table(nont1d_changes$change)
# 56 did not change, 61 did change

# classify people into the group at their earliest visit
nont1d_earliest <- nont1d%>% 
  group_by(Random_ID) %>%
  filter(row_number(Draw_Date)==1)
table(nont1d_earliest$group)
#Multiple ab positive             Negative   Single ab positive 
#                  16                   34                   67 

# classify people into the group at their last visit
nont1d_latest <- nont1d%>% 
  group_by(Random_ID) %>%
  filter(row_number(Draw_Date)==n())
table(nont1d_latest$group)
#Multiple ab positive             Negative   Single ab positive 
#                  25                   36                   56 
