...
# this is the original code for the plot, but now I am generating bootstrapped SE's for the predictions
...
p <- ggplot(long, aes(x=year_floor, y=Pro_pep, colour=final.group)) +
  ...
  geom_line(data=predlong, aes(y=predict(mix_year, level=0, newdata=predlong,interval="confidence")),size=2) +
  labs(colour="Group") +
  theme_bw(base_size=22) 
print(p)
dev.off()
# trying to modify code to add confidence intervals, but ggplot doesn't like the structure of the predictions
png("S:\\Shared Projects\\Laura\\BDC\\Projects\\Andrea Steck\\Trialnet progressors analysis\\Fourth analysis - demethylated insulin with updated dataset\\Abstract\\predicted_over_time.png")
p <- ggplot(long, aes(x=year_floor, y=Pro_pep, colour=final.group)) +
  xlab("Years") + ylab ("PI:C ratio") +
  coord_cartesian(xlim=c(0,15)) +
