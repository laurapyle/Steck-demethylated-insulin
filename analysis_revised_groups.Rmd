...
for(i in 1:nrow(t1d_select_pro)) {
  t1d_select_pro$newgroup[i] <- ifelse(t1d_select_pro$time_from_diagnosis[i]<=90, "T1D",    
...
# UNLESS SUBJECT IS NOT IN THAT CATEGORY AT THAT TIME
...
# keep only records where current group = final group
nont1d_select_pro <- nont1d_select_pro[nont1d_select_pro$group == nont1d_select_pro$final.group,]
check2 <- nont1d_select_pro %>% distinct(Random_ID, .keep_all=TRUE)
# PROBLEM - THERE MAY NOT BE A PROINSULIN RATIO FROM THE SAME VISIT AT WHICH THE PERSON WAS IN THE FINAL GROUP
