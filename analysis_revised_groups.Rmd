...
mix_yeart <- anova.lme(mix_year,type="marginal")
...
# need to reshape preddata so there are two observations for each group
# variable year_floor in first row takes value of min and in second row takes value of max
# then the plot below should work, using start and stop times for each group
# still need to fix formatting
# and then do the same for the adjusted model

test <- predict(mix_year, level=0, newdata=preddata)
...
p <- ggplot(long, aes(x=year_floor, y=Pro_pep, colour=final.group)) +
  ...
  geom_line(data=preddata, aes(y=predict(mix_year, level=0, newdata=preddata)),size=2) +
  labs(colour="Group") +
